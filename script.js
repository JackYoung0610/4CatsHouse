...
function drawStartScreen() {
    ...
    ctx.fillStyle = catColors[currentCatIndex];
    ctx.fillRect(catDisplayX, catDisplayY, catDisplayWidth, catDisplayHeight);
    ...
    ctx.font = '24px sans-serif'; ctx.fillText('點擊後開始用餐', canvas.width / 2, canvas.height * 0.7);
    //ctx.font = '12px sans-serif'; ctx.fillText(' (按 1-5 切換小貓)', canvas.width / 2, canvas.height * 0.7 + 20);
    ctx.textAlign = 'start';
...
function drawGameOver() {
    ...
    ctx.fillText(`${catNames[currentCatIndex]} 得到的分數：${Math.floor(score)}`, canvas.width / 2, canvas.height / 2 - 40);
    ...
    // 繪製底部的操作提示
    ctx.font = '20px sans-serif'; // 增大字體
    ctx.textAlign = 'left';
    const restartText = '重新再用餐';
    const restartTextX = canvas.width * 0.25;
    const restartTextY = canvas.height * 0.8;
    ctx.fillText(restartText, restartTextX, restartTextY);
    const restartTextWidth = ctx.measureText(restartText).width;
    const restartTextHeight = 20; // 與字體大小相關，可調整

    ctx.textAlign = 'right';
    const backToStartText = '回到主畫面';
    const backToStartTextX = canvas.width * 0.75;
    const backToStartTextY = canvas.height * 0.8;
    ctx.fillText(backToStartText, backToStartTextX, backToStartTextY);
    const backToStartTextWidth = ctx.measureText(backToStartText).width;
    const backToStartTextHeight = 20; // 與字體大小相關，可調整

    // 動態計算按鈕區域，增加 padding
    const buttonPaddingX = 30;
    const buttonPaddingY = 15;

    canvas.restartButtonArea = {
        x: restartTextX - restartTextWidth / 2 - buttonPaddingX,
        y: restartTextY - restartTextHeight / 2 - buttonPaddingY,
        width: restartTextWidth + 2 * buttonPaddingX,
        height: restartTextHeight + 2 * buttonPaddingY
    };

    canvas.backToStartButtonArea = {
        x: backToStartTextX - backToStartTextWidth / 2 - buttonPaddingX,
        y: backToStartTextY - backToStartTextHeight / 2 - buttonPaddingY,
        width: backToStartTextWidth + 2 * buttonPaddingX,
        height: backToStartTextHeight + 2 * buttonPaddingY
    };
...
function isClickInsideCat(x, y) {
    if (canvas.catDisplayArea) {
        return x > canvas.catDisplayArea.x &&
               x < canvas.catDisplayArea.x + canvas.catDisplayArea.width &&
               y > canvas.catDisplayArea.y &&
               y < canvas.catDisplayArea.y + canvas.catDisplayArea.height;
    }
    return false;
}
